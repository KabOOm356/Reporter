...
public class Database
    implements DatabaseInterface, ConnectionPooledDatabaseInterface, ConnectionPoolManager {
  ...
  @Override
  public synchronized int openPooledConnection()
      throws ClassNotFoundException, SQLException, InterruptedException {
    return openPooledConnection(null, null);
  }

  private boolean isConnectionSlotAvailable() {
    // A connection slot is available if the size of the pool is less than the max number of
    // connections allowed
    final boolean isConnectionSlotAvailable =
        connectionPool.size() < connectionPoolConfig.getMaxConnections();
    if (log.isDebugEnabled()) {
   ...
   */
  protected void openConnection(final String username, final String password)
  ...
  /**
   * Attempts to open a pooled connection to the database using a username and password.
   *
   * @param username The username to login to the database with.
   * @param password The password for the user associated with the username.
   * @throws SQLException
   ...
   * @throws InterruptedException
   */
  protected synchronized int openPooledConnection(final String username, final String password)
      throws ClassNotFoundException, SQLException, InterruptedException {
    try {
      synchronized (connectionPool) {
        ...
        while (!isConnectionSlotAvailable()) {
          if (!isWaiting) {
            ...
            if (log.isDebugEnabled()) {
              log.warn(
                  String.format(
                      "Thread has been waiting for a new connection for [%dms] this is update [%d]; possible bottleneck!",
                      currentWaitTime, updateCount));
            }
            if (connectionPoolConfig.isConnectionPoolLimited()
                && updateCount >= connectionPoolConfig.getMaxAttemptsForConnection()) {
              log.warn("Thread has reached the max number of updates! Cancelling operation!");
              throw new InterruptedException(
                  String.format(
                      "Thread has reached the cycle limit [%d] after waiting for [%dms] for a new connection!",
                      connectionPoolConfig.getMaxAttemptsForConnection(), currentWaitTime));
            }
          }
        ...
        if (isWaiting && log.isDebugEnabled()) {
          log.debug(
              String.format(
                  "A connection is now available in the connection pool after waiting [%dms]... proceeding!",
                  currentWaitTime));
        }
      }
    ...
    try {
      ...
      final ConnectionWrapper ConnectionWrapper =
          new AlertingPooledConnection(this, connectionId, connection);
  ...
  private void bindParametersToPreparedStatement(
  ...
  private ConnectionWrapper getConnection(final Integer connectionId) {
    ...
    throw new IllegalArgumentException(
        "Connection with connection id [" + connectionId + "] does not exist!");
  }
  ...
  private void openNonPooledConnection()
      throws ClassNotFoundException, SQLException, InterruptedException {
    if (localConnectionId == null) {
      openConnection();
    }
