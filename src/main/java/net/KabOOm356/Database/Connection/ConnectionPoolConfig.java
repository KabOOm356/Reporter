...
public class ConnectionPoolConfig {
  ...
  public ConnectionPoolConfig(
      ...
      final long connectionPoolUpdate,
      final int maxAttemptsForConnection) {
    if (maxConnections < minimumMaxConnections) {
      throw new IllegalArgumentException(
          String.format(
              "Parameter 'maxConnections' cannot be less than [%d]!", minimumMaxConnections));
    }
    if (connectionPoolUpdate < minimumConnectionPoolUpdate) {
      throw new IllegalArgumentException(
          String.format(
              "Parameter 'connectionPoolUpdate' cannot be less than [%d]!",
              minimumConnectionPoolUpdate));
    }
    if (maxAttemptsForConnection < minimumMaxAttemptsForConnection) {
      throw new IllegalArgumentException(
          String.format(
              "Parameter 'maxAttemptsForConnection' cannot be less than [%d]!",
              minimumMaxAttemptsForConnection));
    }
  ...
  /**
   * Returns the maximum number of concurrent connections to allow in the connection pool at one
   * time.
   *
   * @return The maximum number of concurrent connections to allow in the connection pool at one
   *     time.
   */
  ...
  /**
   * Returns the amount of time, in milliseconds, to wait before updating the status of the thread
   * waiting on a new connection.
   *
   * @return The amount of time, in milliseconds, to wait before updating the status of the thread
   *     waiting on a new connection.
   */
