...
public abstract class DatabaseTableVersionUpdater extends DatabaseTableUpdateHandler {
  ...
  public DatabaseTableVersionUpdater(
      final Database database, final String version, final String tableName) {
  ...
  public void update() throws InterruptedException, SQLException, ClassNotFoundException {
    try {
      ...
      if (needsToUpdate()) {
        log.info(
            Reporter.getDefaultConsolePrefix()
                + String.format(
                    "Updating table [%s] to version [%s]...",
                    getTableName(), getDatabaseVersion()));
        apply();
      } else {
        log.info(
            String.format(
                Reporter.getDefaultConsolePrefix() + "Table [%s] is up-to-date with version [%s].",
                getTableName(),
                getDatabaseVersion()));
      }
      ...
      try {
        ...
        log.warn(
            String.format(
                "Failed to commit transaction while updating table [%s] to version [%s]!",
                getTableName(), getDatabaseVersion()));
        throw e;
      }
    }
  ...
  protected abstract boolean needsToUpdate()
      throws InterruptedException, SQLException, ClassNotFoundException;
