...
public class VersionedNetworkFile extends NetworkFile {
   ...
   */
  public VersionedNetworkFile(
      final String fileName, final String version, final String encoding, final String url) {
  ...
  /**
   * Separates the given version String into each version number. <br>
   * <br>
   * Separators are '.', '-', '_', and ' '. These can be used in conjunction with each other. <br>
  ...
  /**
   * Compares the difference between two versions. <br>
   * <br>
   * A returned value of zero mean both versions are equal, positive mean the first version is
   * greater, negative means the second version is greater.
   *
  ...
  /**
   * Returns the version number at the given depth, if there is no version number at the given depth
   * zero is returned.
   *
   * @param depth The depth of the version number to get, starting at 0.
   * @return The version number at the given depth, or zero if there is not a number at the given
   *     depth.
   */
  ...
  /**
   * Separates this {@link VersionedNetworkFile}'s version String into each version number. <br>
   * <br>
   * Separators are '.', '-', '_', and ' '. These can be used in conjunction with each other. <br>
   * EX. 1.2-4, is a valid version.
   *
  ...
  /**
   * Compares this {@link VersionedNetworkFile}'s version to the given {@link
   * VersionedNetworkFile}'s version. <br>
   * <br>
   * A returned value of zero mean both versions are equal, positive mean this version is greater,
   * negative means the given version is greater.
   *
  ...
  public int compareVersionTo(final VersionedNetworkFile comp) {
    ...
    for (int LCV = length - 1; LCV >= 0; LCV--) {
      difference +=
          ((length - LCV) + Math.abs(difference)) * (this.getVersion(LCV) - comp.getVersion(LCV));
    }
  ...
  public enum ReleaseLevel {
    ...
    /**
     * An integer value that is associated with release levels. <br>
     * <br>
     *
     * <p>Full Release = 4. <br>
    ...
    /**
     * Returns the {@link ReleaseLevel} associated with the given name. <br>
     * <br>
    ...
    /**
     * Returns a comparison between this ReleaseLevel's value and the given Object. <br>
     * <br>
     * A returned value of zero mean the both Object's values are equal, positive means this
     * ReleaseLevel's value is greater, negative means the given ReleaseLevel's value is greater.
     *
