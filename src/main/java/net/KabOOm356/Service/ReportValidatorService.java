...
public class ReportValidatorService extends Service {
  ...
  /**
   * Checks if the report index is valid. If it is not an {@link IndexOutOfRangeException} is
   * thrown.
   *
   ...
   */
  public void requireReportIndexValid(final int index)
      throws ClassNotFoundException, SQLException, InterruptedException, IndexOutOfRangeException {
    final int count = getCount();
    if (index < 1 || index > count) {
      final String message =
          String.format("The requested index [%d] is out side of range [1-%d]", index, count);
      throw new IndexOutOfRangeException(message);
    }
  }

  /**
   * Checks if the given report index is valid. Silent version of {@link
   * #requireReportIndexValid(int)} that will not throw a corresponding exception for the reason why
   * the index is not valid. <br>
   * If the report index is not valid, the the given {@link CommandSender} will be alerted.
   *
   ...
   * @throws InterruptedException
   */
  public boolean isReportIndexValid(final int index)
