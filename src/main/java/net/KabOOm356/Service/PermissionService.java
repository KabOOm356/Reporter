...
public class PermissionService extends Service {
  ...
  /**
   * Checks if the given {@link CommandSender} has the given permission node. <br>
   * <br>
   * <b>NOTE:</b> The given {@link CommandSender} will be converted to a {@link Player} first. <br>
   * But, if the given {@link CommandSender} is not a {@link Player}, {@link Boolean#TRUE} will be
   * returned.
   *
   * @param sender The {@link CommandSender} to check if they have the permission node.
   * @param permission The permission node to check.
   * @return True if the given {@link CommandSender} is not a player or has the permission node,
   *     otherwise false.
  ...
  private boolean hasPermissionOverride(final Player player) {
    return getConfiguration().getBoolean("general.permissions.opsHaveAllPermissions", true)
        && player.isOp();
  }
