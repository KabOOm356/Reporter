...
public class ReportPermissionService extends Service {
   ...
   */
  public boolean canAlterReport(
      final CommandSender sender, final int index, final CommandSender player)
    ...
    try {
      ...
      log.error(
          String.format(
              "Failed to check if player [%s] could alter report [%d]!",
              BukkitUtil.formatPlayerName(player), index));
      throw e;
    }
  }

  /**
   * Checks if the given {@link CommandSender} can alter the report at the given index. <br>
   * <br>
   * Will display a message if the sender cannot alter the report. <br>
   * <br>
   ...
   */
  public boolean canAlterReport(final CommandSender sender, final int index)
      throws InterruptedException, SQLException, ClassNotFoundException {
    return canAlterReport(sender, index, sender);
  }

  /**
   * Checks if report is unclaimed or the player has high enough priority to supersede the player
   * claiming it. <br>
   * <br>
   * If the player fails the check, the sender will be alerted.
   *
   * @param sender The {@link CommandSender}.
   * @param index The index of the report.
   * @param player The player to check.
   * @return True if the report is unclaimed or the player has high enough priority to supersede the
   *     player claiming it.
   */
  public boolean requireUnclaimedOrPriority(
      final CommandSender sender, final int index, final CommandSender player)
    ...
    try {
      ...
      final OfflinePlayer claimedByOfflinePlayer =
          BukkitUtil.getOfflinePlayer(claimedByUUID, claimedByName);
      ...
      if (isClaimed
          && !isClaimedBySender
          && !isClaimedByPlayer
          && claimPriority >= getModLevel(player).getLevel()) {
        final String formattedClaimName = BukkitUtil.formatPlayerName(claimedBy);
        final String output =
            getLocale()
                .getString(ClaimPhrases.reportAlreadyClaimed)
  ...
  /**
   * Checks if the given {@link CommandSender} has a high enough priority to alter the report at the
   * given index.
   *
   * @param sender The {@link CommandSender}.
   * @param index The index of the report.
   * @return True if the {@link CommandSender} has a high enough priority to alter the report at the
   *     given index.
   * @throws SQLException
   * @throws ClassNotFoundException
   * @throws InterruptedException
   */
  public boolean checkPriority(final CommandSender sender, final int index)
      throws ClassNotFoundException, InterruptedException, SQLException {
    if (hasPermissionOverride(sender)) {
      return true;
    }
    ...
    try {
      ...
      log.error(
          String.format(
              "Failed to do a priority check for player [%s] on report [%d]",
              BukkitUtil.formatPlayerName(sender), index));
      throw e;
    }
  }
   ...
   * @param index The index of the report.
   * @param player The player to check.
   * @return True if the player has a high enough priority to alter the given report, otherwise
   *     false.
   */
  public boolean requirePriority(
    ...
    try {
      if (!checkPriority(player, index)) {
        ...
        final String output =
            getLocale()
                .getString(GeneralPhrases.reportRequiresClearance)
                .replaceAll("%i", ChatColor.GOLD + Integer.toString(index) + ChatColor.RED)
                .replaceAll(
                    "%m", reportPriority.getColor() + reportPriority.getName() + ChatColor.RED);
  ...
  private ModLevel getReportPriority(final int index)
      throws InterruptedException, SQLException, ClassNotFoundException {
