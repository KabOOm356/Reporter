...
public class SQLStatService extends Service {
   ...
   */
  public SQLStatService(
      final ServiceModule module,
      final String tableName,
      final String indexColumn,
      final String secondaryIndexColumn) {
    super(module);
    if (tableName == null) {
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          log.error(
              String.format(
                  "Failed to increment statistic [%s] by %d for player [%s]!",
                  stat.getName(), increment, BukkitUtil.formatPlayerName(player)),
              e);
        } finally {
        ...
        log.error(
            String.format(
                "Failed to open connection to increment statistic [%s] by %d for player [%s]!",
                stat.getName(), increment, BukkitUtil.formatPlayerName(player)),
            e);
      }
    } catch (final Exception e) {
      log.error(
          String.format(
              "Failed to add row to increment statistic [%s] by %d for player [%s]!",
              stat.getName(), increment, BukkitUtil.formatPlayerName(player)),
          e);
    }
  }
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          log.error(
              String.format(
                  "Failed to decrement statistic [%s] by %d for player [%s]!",
                  stat.getName(), decrement, BukkitUtil.formatPlayerName(player)),
              e);
        } finally {
          getDatabase().closeConnection(connectionId);
        }
        ...
        log.error(
            String.format(
                "Failed to open connection to decrement statistic [%s] by %d for player [%s]!",
                stat.getName(), decrement, BukkitUtil.formatPlayerName(player)),
            e);
      }
    } catch (final Exception e) {
      log.error(
          String.format(
              "Failed to add row to decrement statistic [%s] by %d for player [%s]!",
              stat.getName(), decrement, BukkitUtil.formatPlayerName(player)),
          e);
    }
  }
   ...
   * Sets a statistic. <br>
   * <br>
   * NOTE: This does not create a column for the statistic, it solely sets a value to an already
   * existing column.
   *
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          log.error(
              String.format(
                  "Failed to set statistic [%s] to %s for player [%s]!",
                  stat.getName(), value, BukkitUtil.formatPlayerName(player)),
              e);
        } finally {
          getDatabase().closeConnection(connectionId);
        }
        ...
        log.error(
            String.format(
                "Failed to open connection to set statistic [%s] to %s for player [%s]!",
                stat.getName(), value, BukkitUtil.formatPlayerName(player)),
            e);
      }
    } catch (final Exception e) {
      log.error(
          String.format(
              "Failed to add row to set statistic [%s] to %s for player [%s]!",
              stat.getName(), value, BukkitUtil.formatPlayerName(player)),
          e);
    }
  }
   ...
   * @return An {@link ResultRow} containing the statistic requested. An empty {@link ResultRow} is
   *     returned if there is not an entry for the given player. If an exception is thrown while
   *     querying the database, null is returned.
   */
    ...
    try {
      ...
      try {
        final SQLResultSet result =
            getDatabase().preparedSQLQuery(connectionId, query.toString(), params);
        ...
        log.error(
            String.format(
                "Failed to get SQL stat [%s] for player [%s]!",
                stat.getName(), BukkitUtil.formatPlayerName(player)),
            e);
        return null;
      } finally {
        getDatabase().closeConnection(connectionId);
      ...
      log.error(
          String.format(
              "Failed to open connection to get SQL stat [%s] for player [%s]!",
              stat.getName(), BukkitUtil.formatPlayerName(player)),
          e);
    }
   ...
   * @return A {@link ResultRow} containing the values for the statistics passed. If an exception
   *     occurs while getting a statistic it is omitted from the returned {@link ResultRow}.
   */
   ...
   */
  protected void addRow(final OfflinePlayer player)
    ...
    try {
      ...
      log.warn(
          String.format(
              "Failed to open connection to insert new SQL stat row for player [%s]!",
    ...
    try {
      ...
      log.warn(
          String.format(
              "Failed to insert new SQL stat row for player [%s]!",
              BukkitUtil.formatPlayerName(player)));
      throw e;
    } finally {
      getDatabase().closeConnection(connectionId);
   ...
   * @throws InterruptedException
   */
  protected SQLResultSet getIndex(final OfflinePlayer player)
      throws ClassNotFoundException, SQLException, InterruptedException {
    final int connectionId;
    try {
      connectionId = getDatabase().openPooledConnection();
      ...
      log.warn(
          String.format(
              "Failed to open connection to get SQL stat index for player [%s]!",
              BukkitUtil.formatPlayerName(player)));
      throw e;
    }
    try {
      ...
      try {
        ...
        log.warn(
            String.format(
                "Failed to get SQL stat index for player [%s]!",
                BukkitUtil.formatPlayerName(player)));
        throw e;
      }
    } finally {
