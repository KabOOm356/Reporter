...
public class ReportLimitService extends Service {
  ...
  /**
   * Checks if the given {@link CommandSender} can submit a report. <br>
   * <br>
   * <b>NOTE:</b> If the {@link CommandSender} cannot be converted to a player then true is
   * returned.
   *
  ...
  /**
   * Checks if the given {@link CommandSender} can submit a report against the given {@link
   * OfflinePlayer}. <br>
   * <br>
   * <b>NOTE:</b> If the {@link CommandSender} cannot be converted to a player then true is
   * returned.
   ...
   * @return True if the {@link CommandSender} can submit a report against the given {@link
   *     OfflinePlayer}, otherwise false.
   */
  ...
  public void hasReported(final CommandSender sender, final OfflinePlayer reportedPlayer) {
    ...
    if (isPlayer && canReport && canReportPlayer) {
      ...
      if (!noLimit) {
        ...
        executionTime.add(
            Calendar.SECOND, getConfigurationService().get(ConfigurationEntries.limitTime));

        timer.init(this, player, reportedPlayer, executionTime.getTimeInMillis());

        // Convert from seconds to bukkit ticks
        final long bukkitTicks =
            BukkitUtil.convertSecondsToServerTicks(
                getConfigurationService().get(ConfigurationEntries.limitTime));
        ...
        if (getConfigurationService().get(ConfigurationEntries.alertConsoleWhenLimitReached)
            && !canReport(sender)) {
          final String output =
              "%p has reached their reporting limit!".replaceAll("%p", player.getName());
        ...
        if (getConfigurationService()
                .get(ConfigurationEntries.alertConsoleWhenLimitAgainstPlayerReached)
            && !canReport(sender, reportedPlayer)) {
          final String output =
              "%p has reached their reporting limit for reporting %r!"
   ...
   * @return The seconds remaining before the sender can report the given player again, if the
   *     sender has reported the player. Otherwise, null.
   */
   ...
   *
   * @param sender The sender to get the remaining time for.
   * @return The seconds remaining before the sender can report again, if the sender has reported.
   *     Otherwise, zero (0).
   */
  ...
  /**
   * Called when a {@link ReportTimer} is run, meaning that the time limit for that report has
   * expired.
   *
  ...
  public void limitExpired(final ReportTimer expired) {
    ...
    if (!canReport(player)) {
      ...
      if (getConfigurationService()
          .get(ConfigurationEntries.alertConsoleWhenAllowedToReportAgain)) {
        log.log(
            Level.INFO,
            Reporter.getLogPrefix() + player.getName() + " is now allowed to report again!");
      }
    }

    if (getConfigurationService().get(ConfigurationEntries.limitReportsAgainstPlayers)
        && !canReport(player, reported)) {
      if (getConfigurationService()
          .get(ConfigurationEntries.alertPlayerWhenAllowedToReportPlayerAgain)) {
        final String output =
            getLocale()
                .getString(ReportPhrases.allowedToReportPlayerAgain)
                .replaceAll(
                    "%r", ChatColor.BLUE + BukkitUtil.formatPlayerName(reported) + ChatColor.WHITE);
        player.sendMessage(ChatColor.BLUE + Reporter.getLogPrefix() + ChatColor.WHITE + output);
      }

      if (getConfigurationService()
          .get(ConfigurationEntries.alertConsoleWhenAllowedToReportPlayerAgain)) {
        log.log(
            Level.INFO,
            Reporter.getLogPrefix()
                + String.format(
                    "%s is now allowed to report %s again!",
                    BukkitUtil.formatPlayerName(player), BukkitUtil.formatPlayerName(reported)));
      }
    }
  ...
  private boolean canReport(
      final CommandSender sender, final int limit, final int numberOfReports) {
