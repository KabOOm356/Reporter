...
public class LastViewedReportService extends Service {
  ...
  public int getLastViewed(final CommandSender sender) throws NoLastViewedReportException {
    ...
    if (!hasLastViewed(sender)) {
      throw new NoLastViewedReportException(
          String.format(
              "Sender [%s] does not have a last viewed report!",
              BukkitUtil.formatPlayerName(sender)));
    }
  ...
  /**
   * Gets the last viewed report viewed by the {@link CommandSender} or the parsed int value from
   * the given index.
   *
   * @param sender The {@link CommandSender} that requested their last viewed report.
   * @param index The index the sender specified.
   * @return An int that is either the last report the sender viewed or the parsed value from the
   *     given index.
   * @throws NoLastViewedReportException Thrown if the index equals {@link #lastViewedIndex} but the
   *     {@link CommandSender} has not viewed a report yet.
   * @throws IndexNotANumberException Thrown if the index is not parsable to an integer.
   */
  public int getIndexOrLastViewedReport(final CommandSender sender, final String index)
      throws NoLastViewedReportException, IndexNotANumberException {
    Validate.notNull(sender);
    ...
    if (lastViewedIndex.equalsIgnoreCase(index)) {
      ...
      if (parsedIndex == null) {
        throw new IndexNotANumberException(
            String.format("Could not parse index number from [%s]", index));
      }
      return parsedIndex;
    }
  }
  ...
  /**
   * Removes the record of the {@link CommandSender}'s last report. This should be called when the
   * {@link CommandSender} quits.
   *
  ...
  /**
   * Removes all report indexes that are not contained in the given {@link List} from all players
   * that have viewed them.
   *
