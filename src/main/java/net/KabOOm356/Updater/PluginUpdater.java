...
public class PluginUpdater extends Updater {
   ...
   */
  @Override
  protected VersionedNetworkFile findLatestFile()
    ...
    for (final PluginUpdateMetadata metadata : pluginUpdateMetadataList) {
      ...
      if (Util.startsWithIgnoreCase(currentName, name)) {
        ...
        final VersionedNetworkFile file =
            new VersionedNetworkFile(name + PLUGIN_FILE_EXTENSION, version, link);
        ...
        if (file.getReleaseLevel().compareToByValue(lowestLevel) >= 0) {
          ...
          // If the current file's version is greater than the latestFile's version, set latestFile
          // to the current file.
        ...
        if (latestFile.getVersion() != null) {
          log.log(
              ...
              Reporter.getDefaultConsolePrefix()
                  + "There is a new update available on BukkitDev: Version "
                  + latestFile.getVersion());
        } else {
          log.log(
              Level.WARN,
              Reporter.getDefaultConsolePrefix() + "There is a new update available on BukkitDev!");
        }
      }
