...
public class LocaleUpdater extends Updater {
   ...
   */
  @Override
  public VersionedNetworkFile findLatestFile()
      ...
      if (nNode.getNodeType() == Node.ELEMENT_NODE) {
        ...
        if (Util.startsWithIgnoreCase(UrlIO.getNodeValue(element, "file_name"), name)) {
          ...
          if (file.getReleaseLevel().compareToByValue(lowestLevel) >= 0) {
            ...
            // If the current file's version is greater than the latestFile's version, set
            // latestFile to the current file.
   ...
   * @throws IOException
   */
  public boolean localeDownloadProcess(final File destination)
      throws ParserConfigurationException, SAXException, IOException {
    String localeName = getName();

    if (localeName.contains(ConstantsLocale.LOCALE_FILE_EXTENSION)) {
      localeName =
          localeName.substring(0, localeName.indexOf(ConstantsLocale.LOCALE_FILE_EXTENSION));
    }

    log.log(
        Level.INFO,
        Reporter.getDefaultConsolePrefix() + "Checking for file: " + destination.getName());
    ...
    try {
      ...
      log.log(
          ...
          Reporter.getDefaultConsolePrefix()
              + "Could not find the locale file "
              + localeName
              + ".yml!",
          e);
    }
    ...
    if (downloadFile != null) {
      log.log(
          Level.INFO,
          Reporter.getDefaultConsolePrefix()
              + "Downloading the locale file: "
              + localeName
              + ".yml version "
              + downloadFile.getVersion()
              + "...");
      UrlIO.downloadFile(downloadFile, destination);
      log.log(
          Level.INFO, Reporter.getDefaultConsolePrefix() + "Locale file successfully downloaded.");
      return true;
    }
   ...
   *
   * @return True if the locale file is updated. False if the file is already up to date or failed
   *     to updated.
   * @throws ParserConfigurationException
   * @throws SAXException
   * @throws IOException
   */
  public boolean localeUpdateProcess(final File destination)
      throws ParserConfigurationException, SAXException, IOException {
    String localeName = getName();
    ...
    try {
      ...
      log.log(
          ...
          e);
      log.log(
          Level.WARN, Reporter.getDefaultConsolePrefix() + "Failed to check for locale update!");
    }
    ...
    if (updateNetworkFile == null) {
      ...
      log.log(
          ...
          Reporter.getDefaultConsolePrefix()
              ...
              + getLocalVersion()
              + " to version "
              + updateNetworkFile.getVersion()
              + "...");
      log.log(
          Level.INFO, Reporter.getDefaultConsolePrefix() + "Creating backup of the locale file...");
      ...
      try {
        ...
        log.log(
            Level.WARN, Reporter.getDefaultConsolePrefix() + "Updating the locale file failed.", e);
        destination.delete();
        return false;
      }
