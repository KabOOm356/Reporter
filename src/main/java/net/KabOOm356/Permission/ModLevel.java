...
public enum ModLevel {
  ...
  /**
   * Checks if the given Integer is a valid ModLevel. <br>
   * <br>
  ...
  /**
   * Returns the ModLevel associated with the given String, if the given String can be parsed into a
   * ModLevel by name or if it can be parsed into an Integer then parsed into a valid ModLevel by
   * level.
   *
   ...
   * @return The ModLevel associated with the given level if one exists, otherwise {@link
   *     ModLevel#UNKNOWN} is returned.
   */
   ...
   * @return The ModLevel associated with the given name if one exists, otherwise {@link
   *     ModLevel#UNKNOWN} is returned.
   */
  // Java 1.6 Safe
  public static ModLevel getByName(final String level) {
    ...
    if (level.equalsIgnoreCase(LOW.name) || isInteger && Integer.parseInt(level) == LOW.level) {
      return LOW;
    } else if (level.equalsIgnoreCase(NORMAL.name)
        || isInteger && Integer.parseInt(level) == NORMAL.level) {
      return NORMAL;
    } else if (level.equalsIgnoreCase(HIGH.name)
        || isInteger && Integer.parseInt(level) == HIGH.level) {
      return HIGH;
    } else if (level.equalsIgnoreCase(NONE.name)
        || isInteger && Integer.parseInt(level) == NONE.level) {
      return NONE;
    }
    return UNKNOWN;
  }
  ...
  /**
   * Compares two ModLevels by their values. <br>
   * <br>
   * Returns zero if both values are equal, positive if the first ModLevel's value is greater than
   * the second ModLevel's value, or negative if the second ModLevel's value is greater than the
   * first's.
   *
  ...
  /**
   * Compares this ModLevel with the given ModLevel. <br>
   * <br>
   * Returns zero if both values are equal, positive if this ModLevel's value is greater than the
   * given ModLevel's value, or negative if the given ModLevel's value is greater.
   *
