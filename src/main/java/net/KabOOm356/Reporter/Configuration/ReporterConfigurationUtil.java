...
public final class ReporterConfigurationUtil {
  ...
  /**
   * Initializes the Reporter configuration file by extracting the file if it does not exist and
   ...
   * @param configuration A {@link FileConfiguration} that will be loaded with the configuration
   *     file.
   */
  public static void initConfiguration(
      final URL defaultConfigurationFile,
    ...
    if (!configFile.exists()) {
      ...
      try {
        log.log(
            Level.INFO,
            Reporter.getDefaultConsolePrefix() + "Extracting default config file from the jar.");

        configFile.createNewFile();
        final InputStreamReader inputStreamReader =
            new InputStreamReader(defaultConfigurationFile.openStream());
  ...
  /**
   * Checks the configuration for updates and then updates if needed. <br>
   * <br>
  ...
  public static boolean updateConfiguration(final FileConfiguration configuration) {
    ...
    // Version 6
    updated =
        set(configuration, "locale.locale", configuration.getString("general.locale", "en_US"))
            || updated;
    ...
    updated =
        set(
                configuration,
                "general.messaging.listOnLogin.listOnLogin",
                configuration.getBoolean("general.messaging.listOnLogin", true))
            || updated;
    ...
    updated =
        set(
                configuration,
                "general.messaging.completedMessageOnLogin.completedMessageOnLogin",
                true)
            || updated;
    updated =
        set(configuration, "general.messaging.completedMessageOnLogin.useDelay", true) || updated;
    ...
    updated =
        set(
                configuration,
                "plugin.updates.checkForUpdates",
                configuration.getBoolean("general.checkForUpdates", true))
            || updated;
    updated = set(configuration, "plugin.updates.releaseLevel", "RELEASE") || updated;
    updated =
        set(
                configuration,
                "locale.updates.autoDownload",
                configuration.getBoolean("locale.localeAutoDownload", true))
            || updated;
    updated =
        set(
                configuration,
                "locale.updates.keepBackup",
                configuration.getBoolean("locale.keepLocaleBackupFile", false))
            || updated;
    ...
    updated =
        set(configuration, "general.reporting.alerts.toConsole.limitAgainstPlayerReached", true)
            || updated;
    updated =
        set(configuration, "general.reporting.alerts.toConsole.allowedToReportPlayerAgain", true)
            || updated;
    updated =
        set(configuration, "general.reporting.alerts.toPlayer.allowedToReportAgain", true)
            || updated;
    updated =
        set(configuration, "general.reporting.alerts.toPlayer.allowedToReportPlayerAgain", true)
            || updated;
    updated =
        set(
                configuration,
                "general.reporting.alerts.toConsole.limitReached",
                configuration.getBoolean("general.reporting.alerts.limitReached", true))
            || updated;
    updated =
        set(
                configuration,
                "general.reporting.alerts.toConsole.allowedToReportAgain",
                configuration.getBoolean("general.reporting.alerts.allowedToReportAgain", true))
            || updated;
    ...
    updated =
        set(
                configuration,
                "database.connectionPool.enableLimiting",
                ConnectionPoolConfig.defaultInstance.isConnectionPoolLimited())
            || updated;
    updated =
        set(
                configuration,
                "database.connectionPool.maxNumberOfConnections",
                ConnectionPoolConfig.defaultInstance.getMaxConnections())
            || updated;
    updated =
        set(
                configuration,
                "database.connectionPool.maxNumberOfAttemptsForConnection",
                ConnectionPoolConfig.defaultInstance.getMaxAttemptsForConnection())
            || updated;
    updated =
        set(
                configuration,
                "database.connectionPool.waitTimeBeforeUpdate",
                ConnectionPoolConfig.defaultInstance.getWaitTimeBeforeUpdate())
            || updated;
    updated =
        set(configuration, "general.messaging.alerts.reportedPlayerLogin.enabled", true) || updated;
    updated =
        set(configuration, "general.messaging.alerts.reportedPlayerLogin.toPlayer", true)
            || updated;
    updated =
        set(configuration, "general.messaging.alerts.reportedPlayerLogin.toConsole", true)
            || updated;
    ...
    if (updated) {
      configuration
          ...
          .header(
              "Reporter Configuration File\n"
                  ...
                  + '\n'
                  + "Config Version: "
                  + Reporter.getConfigurationVersion()
                  + '\n'
      ...
      log.log(
          ...
          Reporter.getDefaultConsolePrefix()
              + "Updating the config file to version "
              + Reporter.getConfigurationVersion());
    }
  ...
  private static boolean set(
      final FileConfiguration configuration, final String key, final Object value) {
