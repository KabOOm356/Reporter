  ...
  protected void apply() throws SQLException, ClassNotFoundException, InterruptedException {
    ...
    if (!getColumns().contains("ID")) {
      addQueryToTransaction(
          "ALTER TABLE PlayerStats ADD ID INTEGER PRIMARY KEY"
    ...
    if (!getColumns().contains("FirstReportDate")) {
      addQueryToTransaction(
          "ALTER TABLE PlayerStats ADD FirstReportDate VARCHAR(19) NOT NULL DEFAULT ''");
    }
    if (!getColumns().contains("LastReportDate")) {
      addQueryToTransaction(
          "ALTER TABLE PlayerStats ADD LastReportDate VARCHAR(19) NOT NULL DEFAULT ''");
    }
    ...
    if (!getColumns().contains("FirstReportedDate")) {
      addQueryToTransaction(
          "ALTER TABLE PlayerStats ADD FirstReportedDate VARCHAR(19) NOT NULL DEFAULT ''");
    }
    if (!getColumns().contains("LastReportedDate")) {
      addQueryToTransaction(
          "ALTER TABLE PlayerStats ADD LastReportedDate VARCHAR(19) NOT NULL DEFAULT ''");
    }
    if (!getColumns().contains("ReportedCount")) {
      addQueryToTransaction(
          "ALTER TABLE PlayerStats ADD ReportedCount INTEGER NOT NULL DEFAULT '0'");
    }
  }
}
