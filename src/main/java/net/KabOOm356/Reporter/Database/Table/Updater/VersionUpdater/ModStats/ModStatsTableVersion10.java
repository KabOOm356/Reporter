...
public class ModStatsTableVersion10 extends DatabaseTableVersionUpdater {
	...
	protected void apply() throws SQLException, ClassNotFoundException, InterruptedException {
		...
		if (!getColumns().contains("ID")) {
			addQueryToTransaction("ALTER TABLE ModStats ADD ID INTEGER PRIMARY KEY" + DatabaseUtil.getAutoIncrementingPrimaryKeySuffix(getDatabase()));
		}
		...
		if (!getColumns().contains("CompletionCount")) {
			addQueryToTransaction("ALTER TABLE ModStats ADD CompletionCount INTEGER NOT NULL DEFAULT '0'");
		}
