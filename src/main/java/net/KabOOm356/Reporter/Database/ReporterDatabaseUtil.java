...
public final class ReporterDatabaseUtil {
   ...
   *
   * @param configuration The {@link FileConfiguration} that will be used to get the database
   *     options.
   ...
   */
  public static ExtendedDatabaseHandler initDB(
      final FileConfiguration configuration, final File dataFolder)
    ...
    final boolean connectionPoolLimit =
    ...
    try {
      connectionPoolConfig =
          new ConnectionPoolConfig(
              ...
              maxNumberOfAttemptsForConnection);
    ...
    if (configuration
        .getString("database.type", DatabaseType.SQLITE.toString())
        .equalsIgnoreCase(DatabaseType.MYSQL.toString())) {
      try {
        ...
        databaseHandler =
            new ExtendedDatabaseHandler(host, database, username, password, connectionPoolConfig);
        ...
        log.log(
            Level.ERROR,
            Reporter.getDefaultConsolePrefix() + "Error connecting to MySQL server using SQLite.",
            e);
      }
    } else {
      fallbackToNextDB = true;
    ...
    if (fallbackToNextDB) {
      ...
      try {
        databaseHandler =
            new ExtendedDatabaseHandler(
                DatabaseType.SQLITE, dataFolder.getPath(), databaseName, connectionPoolConfig);
        initDatabaseTables(databaseHandler.getDatabase());
   ...
   * @throws InterruptedException
   */
  private static void initDatabaseTables(final Database database)
  ...
  private static void checkConnection(final Database database)
