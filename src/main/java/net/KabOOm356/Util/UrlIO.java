...
public final class UrlIO {
   ...
   * @throws IOException
   */
  public static void downloadFile(final NetworkFile abstractFile, final File destination)
      throws IOException {
    if (abstractFile == null && destination == null) {
      throw new IllegalArgumentException(
          "Both the abstract file and destination file cannot be null!");
    ...
    if (!isResponseValid(connection)) {
      throw new IOException(
          String.format("Connection response [%d] is not valid!", connection.getResponseCode()));
    }
    ...
    try {
      in =
          new BufferedReader(
              new InputStreamReader(connection.getInputStream(), abstractFile.getEncoding()));
      out =
          new BufferedWriter(
              new OutputStreamWriter(
                  Files.newOutputStream(destination.toPath()), outputCharset.name()));
  ...
  public static String getVersion(String fileName) {
    if (fileName.contains(" v")) {
      ...
      if (version.equalsIgnoreCase(ALPHA)
          ...
          || version.equalsIgnoreCase(RELEASE_CANDIDATE)) {
