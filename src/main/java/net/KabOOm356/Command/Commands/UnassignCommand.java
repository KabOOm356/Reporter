...
public class UnassignCommand extends ReporterCommand {
  ...
  private static final List<Usage> usages =
      Collections.unmodifiableList(
          ArrayUtil.arrayToList(
              new Usage[] {
                new Usage(UnassignPhrases.unassignHelp, UnassignPhrases.unassignHelpDetails)
              }));
  ...
  private void unassignReport(final CommandSender sender, final int index)
    ...
    try {
      ...
      log.error(
          String.format("Failed to execute unassign query on connection [%s]!", connectionId));
      throw e;
    } finally {
    ...
    if (claimingPlayer != null && claimingPlayer.isOnline()) {
      ...
      claimingPlayer
          .getPlayer()
          .sendMessage(ChatColor.BLUE + Reporter.getLogPrefix() + ChatColor.RED + output);
    }
  }
}
