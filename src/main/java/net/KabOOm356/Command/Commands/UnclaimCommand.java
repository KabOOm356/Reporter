...
public class UnclaimCommand extends ReporterCommand {
  ...
  private static final List<Usage> usages =
      Collections.unmodifiableList(
          ArrayUtil.arrayToList(
              new Usage[] {
                new Usage(UnclaimPhrases.unclaimHelp, UnclaimPhrases.unclaimHelpDetails)
              }));
  ...
  private boolean canUnclaimReport(final CommandSender sender, final int index)
      throws ClassNotFoundException, SQLException, InterruptedException {
    final String query =
        "SELECT ClaimStatus, ClaimedByUUID, ClaimedBy FROM Reports WHERE ID=" + index;
    ...
    try {
      ...
      if (result.getBoolean("ClaimStatus")) {
        ...
        if (!senderIsClaimingPlayer) {
          ...
          output =
              output.replaceAll("%i", ChatColor.GOLD + Integer.toString(index) + ChatColor.RED);
      ...
      log.error(
          String.format(
              "Failed to determine if player can unclaim report on connection [%s]!",
              connectionId));
      throw e;
    } finally {
  ...
  private void unclaimReport(final CommandSender sender, final int index)
      throws ClassNotFoundException, SQLException, InterruptedException {
    final String query =
        "UPDATE Reports "
            + "SET "
