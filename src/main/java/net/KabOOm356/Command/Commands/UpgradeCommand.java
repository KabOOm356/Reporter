...
public class UpgradeCommand extends ReporterCommand {
  ...
  private static final List<Usage> usages =
      Collections.unmodifiableList(
          ArrayUtil.arrayToList(
              new Usage[] {
                new Usage(UpgradePhrases.upgradeHelp, UpgradePhrases.upgradeHelpDetails)
              }));
  ...
  public void execute(final CommandSender sender, final List<String> args)
    ...
    try {
      ...
      if (newPriority == ModLevel.UNKNOWN) {
        String output =
            getManager().getLocale().getString(UpgradePhrases.reportIsAtHighestPriority);
  ...
  private ModLevel getNextPriorityLevel(final int index)
    ...
    try {
      ...
      log.error(
          String.format(
              "Failed to execute query to get next highest priority on connection [%s]!",
              connectionId));
      throw e;
    } finally {
