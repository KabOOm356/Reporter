...
public class AssignCommand extends ReporterCommand {
  ...
  private static final List<Usage> usages =
      Collections.unmodifiableList(
          ArrayUtil.arrayToList(
              new Usage[] {new Usage(AssignPhrases.assignHelp, AssignPhrases.assignHelpDetails)}));
  ...
  private void assignReport(final CommandSender sender, final int index, final Player player)
      throws ClassNotFoundException, SQLException, InterruptedException {
    final String query =
        "UPDATE Reports SET ClaimStatus=?, ClaimDate=?, ClaimedBy=?, ClaimedByUUID=?, ClaimPriority=? WHERE ID=?";
    ...
    params.add(
        4, Integer.toString(getServiceModule().getPlayerService().getModLevel(player).getLevel()));
  ...
  /**
   * Checks if the given {@link CommandSender} can assign the given player to the report at the
   * given index.
   *
   ...
   */
  public boolean canAssignReport(final CommandSender sender, final int index, final Player player)
    ...
    if (!senderIsConsoleOrOp && senderHasLowerModLevel) {
      ...
      output =
          output.replaceAll("%m", playerLevel.getColor() + playerLevel.getName() + ChatColor.WHITE);
      sender.sendMessage(ChatColor.WHITE + output);
