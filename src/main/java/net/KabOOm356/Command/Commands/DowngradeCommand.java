...
public class DowngradeCommand extends ReporterCommand {
  ...
  private static final List<Usage> usages =
      Collections.unmodifiableList(
          ArrayUtil.arrayToList(
              new Usage[] {
                new Usage(DowngradePhrases.downgradeHelp, DowngradePhrases.downgradeHelpDetails)
              }));
  ...
  public void execute(final CommandSender sender, final List<String> args)
    ...
    try {
      ...
      if (newPriority == ModLevel.UNKNOWN) {
        String output =
            getManager().getLocale().getString(DowngradePhrases.reportIsAtLowestPriority);
  ...
  private ModLevel getNextLowestPriorityLevel(final int index)
    ...
    try {
      ...
      log.error(
          String.format(
              "Failed to get the next lowest priority on connection [%d]!", connectionId));
      throw e;
    } finally {
