...
public class MoveCommand extends ReporterCommand {
  ...
  private static final List<Usage> usages =
      Collections.unmodifiableList(
          ArrayUtil.arrayToList(
              new Usage[] {new Usage(MovePhrases.moveHelp, MovePhrases.moveHelpDetails)}));
  ...
  protected void moveReport(final CommandSender sender, final int index, final ModLevel level)
    ...
    try {
      ...
      if (isClaimed && level.getLevel() > currentPriority) {
        ...
        if (claimingPlayer != null) {
          ...
          claimingPlayer.sendMessage(
              ChatColor.BLUE + Reporter.getLogPrefix() + ChatColor.RED + output);
        }
      ...
      log.log(
          Level.ERROR,
          String.format("Failed to move report priority on connection [%d]!", connectionId));
      throw e;
    } finally {
