...
public final class HelpCommand extends ReporterCommand {
  ...
  private static final String permissionNode = "reporter.help";
  ...
  public HelpCommand(
      ...
      final Locale locale,
      final Collection<ReporterCommand> commands,
      final HelpCommandDisplay display) {
  ...
  private void printFooter(final CommandSender sender, final int page) {
    if (page != pages.size()) {
      final String nextPage =
          getLocale().getString(display.getNext()).replaceAll("%p", Integer.toString(page + 1));
  ...
  private void printPage(final CommandSender sender, final int page) {
    ...
    for (final Usage usage : currentPage) {
      ...
      sender.sendMessage(
          format.replaceAll("%usage", usageString).replaceAll("%description", description));
    }
  }
