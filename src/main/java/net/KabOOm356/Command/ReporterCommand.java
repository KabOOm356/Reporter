...
public abstract class ReporterCommand extends TimedRunnable implements RunnableWithState {
  ...
  public void hasRequiredPermission(final CommandSender sender) throws RequiredPermissionException {
    if (!hasPermission(sender)) {
      throw new RequiredPermissionException(
          String.format(
              "Sender [%s] does not have the required permission [%s]!",
              BukkitUtil.formatPlayerName(sender), permissionNode));
    }
  }
  ...
  public String getErrorMessage() {
    return ChatColor.BLUE
        ...
        + ChatColor.RED
        + manager.getLocale().getString(GeneralPhrases.error);
  }
  ...
  public void setSender(final CommandSender sender) {
    if (isRunning || isPendingToRun) {
      throw new IllegalArgumentException(
          "The current command is in-flight and should not be modified!");
    }
    this.sender = sender;
  }
  ...
  public ReporterCommand getRunnableClone(final CommandSender sender, final List<String> arguments)
      throws Exception {
    try {
      final Class<? extends ReporterCommand> clazz = this.getClass();
      final Constructor<? extends ReporterCommand> constructor =
          clazz.getDeclaredConstructor(ReporterCommandManager.class);
